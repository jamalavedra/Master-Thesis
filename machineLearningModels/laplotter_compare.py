...
def ignore_nan_and_inf(value, label, x_index):
    ...
    if value is None:
        ...
        warnings.warn("Got NaN for value '%s' at x-index %d" %
                      (label, x_index))
        return None
    elif math.isinf(value):
        warnings.warn("Got INF for value '%s' at x-index %d" %
                      (label, x_index))
        return None
    else:
        return value
...
class ComparePlotter(object):
    ...
    def __init__(self,
        ...
        self.linestyles = {
            ...
            "loss_val1": "b-",
            "loss_val_sma1": "b-",
            "loss_val_regression1": "b:",
            "loss_val2": "g-",
            "loss_val_sma2": "g-",
            "loss_val_regression2": "g:",
            "acc_train": "r-",
            "acc_train_sma": "r-",
            "acc_train_regression": "r:",
            "acc_val1": "b-",
            "acc_val_sma1": "b-",
            "acc_val_regression1": "b:",
            "acc_val2": "g-",
            "acc_val_sma2": "g-",
            "acc_val_regression2": "g:"
        }
        ...
        self.values_loss_val1 = OrderedDict()
        self.values_loss_val2 = OrderedDict()
        self.values_acc_train = OrderedDict()
        self.values_acc_val1 = OrderedDict()
        self.values_acc_val2 = OrderedDict()

    def add_values(self, x_index, loss_train=None, loss_val1=None, loss_val2=None, acc_train=None,
                   acc_val1=None, acc_val2=None, redraw=True):
        ...
        loss_val1 = ignore_nan_and_inf(loss_val1, "loss val", x_index)
        loss_val2 = ignore_nan_and_inf(loss_val2, "loss val", x_index)
        acc_train = ignore_nan_and_inf(acc_train, "acc train", x_index)
        acc_val1 = ignore_nan_and_inf(acc_val1, "acc val", x_index)
        acc_val2 = ignore_nan_and_inf(acc_val2, "acc val", x_index)

        if loss_train is not None:
            self.values_loss_train[x_index] = loss_train
        if loss_val1 is not None:
            self.values_loss_val1[x_index] = loss_val1
        if loss_val2 is not None:
            self.values_loss_val2[x_index] = loss_val2
        if acc_train is not None:
            self.values_acc_train[x_index] = acc_train
        if acc_val1 is not None:
            self.values_acc_val1[x_index] = acc_val1
        if acc_val2 is not None:
            self.values_acc_val2[x_index] = acc_val2
    ...
    def redraw(self):
        ...
        for ax, label in zip([ax1, ax2], ["X axis displacement (m)", "Y axis displacement (m)"]):
            if ax:
                ax.clear()
                ax.set_title(label)
                ax.set_ylabel(label)
        ...
        ncol = 1
        labels = ["real $CHART ", "SIFT pred.  $CHART ", "CF pred.  $CHART "]
        if self.show_averages:
            labels.extend(["real $CHART (avg %d)" % (self.averages_period,),
                           "SIFT pred. $CHART (avg %d)" % (
                               self.averages_period,),
                           "CF pred. $CHART (avg %d)" % (self.averages_period,)])
            ncol += 1

        if ax1:
            ax1.legend([label.replace("$CHART", "displacement X") for label in labels],
                       loc="upper center",
                       prop={'size': 15},
                       bbox_to_anchor=(0.5, -0.08),
                       ncol=ncol)
        if ax2:
            ax2.legend([label.replace("$CHART", "displacement Y") for label in labels],
                       loc="upper center",
                       prop={'size': 15},
                       bbox_to_anchor=(0.5, -0.08),
                       ncol=ncol)
    ...
    def _redraw_main_lines(self):
        ...
        ls_loss_val1 = self.linestyles["loss_val1"]
        ls_loss_val2 = self.linestyles["loss_val2"]
        ls_acc_train = self.linestyles["acc_train"]
        ls_acc_val1 = self.linestyles["acc_val1"]
        ls_acc_val2 = self.linestyles["acc_val2"]

        if len(self.values_loss_train) == 1:
            ls_loss_train = self.linestyles_one_value["loss_train"]
        if len(self.values_loss_val1) == 1:
            ls_loss_val1 = self.linestyles_one_value["loss_val1"]
        if len(self.values_loss_val2) == 1:
            ls_loss_val2 = self.linestyles_one_value["loss_val2"]
        if len(self.values_acc_train) == 1:
            ls_acc_train = self.linestyles_one_value["acc_train"]
        if len(self.values_acc_val1) == 1:
            ls_acc_val1 = self.linestyles_one_value["acc_val1"]
        if len(self.values_acc_val2) == 1:
            ls_acc_val2 = self.linestyles_one_value["acc_val2"]
        ...
        if ax1:
            ...
            h_lv1, = ax1.plot(list(self.values_loss_val1.keys()), list(self.values_loss_val1.values()),
                              ls_loss_val1, label="loss val.", alpha=alpha_main)
            h_lv2, = ax1.plot(list(self.values_loss_val2.keys()), list(self.values_loss_val2.values()),
                              ls_loss_val2, label="loss val.", alpha=alpha_main)
            handles.extend([h_lt, h_lv1, h_lv2])
        if ax2:
            ...
            h_av1, = ax2.plot(list(self.values_acc_val1.keys()), list(self.values_acc_val1.values()),
                              ls_acc_val1, label="acc. val.", alpha=alpha_main)
            h_av2, = ax2.plot(list(self.values_acc_val2.keys()), list(self.values_acc_val2.values()),
                              ls_acc_val2, label="acc. val.", alpha=alpha_main)
            handles.extend([h_at, h_av1, h_av2])
    ...
    def _redraw_averages(self):
        ...
        if ax1:
            ...
            (lv1_sma_x, lv1_sma_y) = self._calc_sma(self.values_loss_val1.keys(),
                                                    self.values_loss_val1.values())
            (lv2_sma_x, lv2_sma_y) = self._calc_sma(self.values_loss_val2.keys(),
                                                    self.values_loss_val2.values())
        if ax2:
            # for accuracy chart
            (at_sma_x, at_sma_y) = self._calc_sma(self.values_acc_train.keys(),
                                                  self.values_acc_train.values())
            (av1_sma_x, av1_sma_y) = self._calc_sma(self.values_acc_val1.keys(),
                                                    self.values_acc_val1.values())
            (av2_sma_x, av2_sma_y) = self._calc_sma(self.values_acc_val2.keys(),
                                                    self.values_acc_val2.values())
        # plot the xy-values
        alpha_sma = self.alpha_thick
        if ax1:
            # for loss chart
            h_lt, = ax1.plot(lt_sma_x, lt_sma_y, self.linestyles["loss_train_sma"],
                             label="train loss (avg %d)" % (
                                 self.averages_period,),
                             alpha=alpha_sma)
            h_lv1, = ax1.plot(lv1_sma_x, lv1_sma_y, self.linestyles["loss_val_sma1"],
                              label="val loss (avg %d)" % (
                self.averages_period,),
                alpha=alpha_sma)
            h_lv2, = ax1.plot(lv2_sma_x, lv2_sma_y, self.linestyles["loss_val_sma2"],
                              label="val loss (avg %d)" % (
                self.averages_period,),
                alpha=alpha_sma)
            handles.extend([h_lt, h_lv1, h_lv2])
        if ax2:
            ...
            h_at, = ax2.plot(at_sma_x, at_sma_y, self.linestyles["acc_train_sma"],
                             label="train acc (avg %d)" % (
                                 self.averages_period,),
                             alpha=alpha_sma)
            h_av1, = ax2.plot(av1_sma_x, av1_sma_y, self.linestyles["acc_val_sma1"],
                              label="acc. val. (avg %d)" % (
                self.averages_period,),
                alpha=alpha_sma)
            h_av2, = ax2.plot(av2_sma_x, av2_sma_y, self.linestyles["acc_val_sma2"],
                              label="acc. val. (avg %d)" % (
                self.averages_period,),
                alpha=alpha_sma)
            handles.extend([h_at, h_av1, h_av2])
